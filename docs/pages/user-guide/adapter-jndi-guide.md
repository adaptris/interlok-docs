...
Then during initialization of the shared-components you will see further debug logging informing you of each JNDI name that has been bound. Enabling log4j trace level logging will log every request to lookup an object.
 ...
 ![jndi-logging-1](../../images/jndi/jndi-logging-1.png)
...
?> **NOTE** As a rule, if the shared components are only referenced inside Interlok configuration, then the unique-id is all that is required; specifying the lookup name can sometimes lead to unexpected behaviour within the UI.
...
?> **TIP** You should only specify the lookup name if you have non-Interlok components hosted inside the Interlok JVM accessing the component.
...
?> **NOTE** Specifying a `unique-id` of `mydatasource` will expose both the DatabaseConnection under the name `mydatasource` and the underlying DataSource under `comp/env/jdbc/mydatasource`.
...
!> **IMPORTANT** You must set `enableLocalJndiServer=true` in your _bootstrap.properties_ before this becomes available.
...
[JmsConnection]: https://nexus.adaptris.net/nexus/content/sites/javadocs/com/adaptris/interlok-core/5.0-SNAPSHOT/com/adaptris/core/jms/JmsConnection.html
[DatabaseConnection]: https://nexus.adaptris.net/nexus/content/sites/javadocs/com/adaptris/interlok-core/5.0-SNAPSHOT/com/adaptris/core/jdbc/DatabaseConnection.html
[JdbcConnection]: https://nexus.adaptris.net/nexus/content/sites/javadocs/com/adaptris/interlok-core/5.0-SNAPSHOT/com/adaptris/core/jdbc/JdbcConnection.html
[JdbcDataQueryService]: https://nexus.adaptris.net/nexus/content/sites/javadocs/com/adaptris/interlok-core/5.0-SNAPSHOT/com/adaptris/core/services/jdbc/JdbcDataQueryService.html
